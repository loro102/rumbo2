...
$cia_cmd_CommandText="SELECT * FROM Aseguradoras WHERE aseguradora LIKE ?";
$cia_cmd_Prepared=true;
$cia_cmd_Parameters=$Append$cia_cmd_CreateParameter="param1"$cia__MMColParam+"%"); // adVarChar
...
//set b=fso.OpenTextFile("C:\\Documentos\\Plantilla\\"+request.QueryString("Contrato")+".rtf",1,false,0)
...
$a=str_replace("--==CODIGOCUENTA==--",(->$Item["CCC"]->$Value),$a);
...
$a=str_replace("--==CONDICION==--",(->$Item["Condicion"]->$Value),$a);
$a=str_replace("--==REF.EXP==--",(->$Item["RefExpediente"]->$Value),$a);
$a=str_replace("--==FDESIGNA==--",(->$Item["Fecha Designacion"]->$Value),$a);
$a=str_replace("--==TRAMITADOR==--",(->$Item["nombre"]->$value),$a);
